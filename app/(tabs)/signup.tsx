...
const SignupScreen = () => {
  ...
  const { strengthColor, strengthLabel } = useMemo(() => {
    if (passwordStrength <= 2) return { strengthColor: COLORS.weak, strengthLabel: "Weak" };
    if (passwordStrength <= 4) return { strengthColor: COLORS.medium, strengthLabel: "Medium" };
    ...
    <Wrapper>
      <View className="flex-1 w-full bg-white px-6 justify-start mt-8">
        ...
        <CustomTextInput
          ...
          showValidationIcon={showAllErrors && !!errors.password && !!formData.password}
            ...
            <View className="flex-row h-1.5 bg-gray-200 rounded-full overflow-hidden">
              {Array.from({ length: 5 }).map((_, index) => (
                <View
                  ...
                  style={{
                    backgroundColor: index < passwordStrength ? strengthColor : COLORS.gray,
