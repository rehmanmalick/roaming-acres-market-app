...
const MessageComponent = ({ name, text, time, message, noOfMessage , source, active}: MessageComponentProps) => {
  return (
    <TouchableOpacity className="flex-row bg-white rounded-lg p-2 my-2">
      ...
      <View className="flex-row items-center mb-1">
        <Text className={`text-sm ${message ? 'text-black' : 'text-gray-400'}`}>{text}</Text>
      </View>
      </View>
      
      <View className="pl-6 items-center justify-center"> 
      <Text className={`text-md font-semibold ${message ? 'text-black' : 'text-gray-400'} mb-1`}>{time}</Text>
      {message && (
        <View className="h-6 w-6">
        <Text style={{ backgroundColor: "orange" }} className="rounded-xl text-white text-center text-sm font-bold p-1">
          {noOfMessage}
        </Text>
        </View>
      )}
