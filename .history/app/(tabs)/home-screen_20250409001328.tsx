...
import { View, Text, ScrollView, TextInput, TouchableOpacity, Modal, Platform } from 'react-native';
    ...
    >
      <View className="flex-1 justify-end bg-black/50">
        <View className="bg-white rounded-t-3xl p-6" style={{ maxHeight: '80%' }}>
          ...
          >
            <View className={`p-2 rounded-full ${selectedLocation === 'current' ? 'bg-teal-100' : 'bg-gray-100'}`}>
              <Ionicons 
            ...
            <View className="ml-3">
              <Text className={`text-base ${selectedLocation === 'current' ? 'font-semibold text-teal-600' : ''}`}>
                Use my current location
              </Text>
              <Text className="text-xs text-gray-500">Using GPS to find your exact location</Text>
            </View>
          </TouchableOpacity>
          ...
          {[1, 2, 3].map((item) => (
            ...
            >
              <View className={`p-2 rounded-full ${selectedLocation === `home${item}` ? 'bg-teal-100' : 'bg-gray-100'}`}>
                <FontAwesome6 
                  name="house" 
                  size={16} 
                  color={selectedLocation === `home${item}` ? '#008080' : '#666'} 
                />
              </View>
              <View className="ml-3">
                <Text className={`text-base ${selectedLocation === `home${item}` ? 'font-semibold text-teal-600' : ''}`}>
        ...
        <Wrapper showFilterButton={true} showMenuButton={true}>
          ...
          <View className="p-4">
            <View className="flex-row justify-between items-center">
              <View>
                ...
                <Text className="text-3xl font-bold mt-1">Roaming Acres Market</Text>
              </View>
              <TouchableOpacity onPress={()=> router.push('/notification-setting')} className="relative">
                <Ionicons 
                  name="notifications" 
                  size={24} 
                  color="white" 
                  style={{ backgroundColor: '#008080', borderRadius: 12, padding: 4 }} 
                />
                <View className="absolute bg-orange-500 rounded-full" style={{ width: 10, height: 10, top: -2, right: -2 }} />
              </TouchableOpacity>
            </View>

            <View className="mt-4 relative">
              <TextInput
                className="border border-gray-300 rounded-full py-4 px-4 pl-12"
                placeholder="Search"
          ...
          <View className="mt-8 px-4">
            ...
            <ScrollView 
              ...
              className="mt-2 "
            >
              <View className="flex-row item-center justify-center space-x-4">
                <NewArrival price={50} />
                <NewArrival price={50} />
                <NewArrival price={50} />
              </View>
