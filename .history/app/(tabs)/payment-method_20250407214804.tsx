...
const CreditCardForm = () => {
  ...
  const formatExpiry = (input: string): string => {
    // Remove all non-digit characters
    const cleaned = input.replace(/\D/g, "");
    // Add slash after 2 digits (month)
    if (cleaned.length > 2) {
        ...
        <TouchableWithoutFeedback onPress={Keyboard.dismiss}>
          <View className="flex-1 w-full bg-white px-6 justify-start mt-8">
            ...
            <CustomTextInput
              ...
              validationIconColor={COLORS.success}
              showValidationIcon={!!formData.cardHolder && !errors.cardHolder}
            />

            <CustomTextInput
              className="bg-[##F0F5FA] w-full h-14 border  rounded-[3px] px-4 text-base "
              cursorColor="red"
              selectionColor="blue"
              label="Card Number"
              ref={cardNumberRef}
              ...
              validationIconColor={COLORS.success}
              showValidationIcon={!!formData.cardNumber && !errors.cardNumber}
            />
            ...
            <View className="flex-row justify-between">
              <View className="w-[48%]">
                <CustomTextInput
                  ...
                  validationIcon={
                    formData.expiry && !errors.expiry
                      ? "checkmark-circle"
                      : undefined
                  }
                  validationIconColor={COLORS.success}
                  showValidationIcon={!!formData.expiry && !errors.expiry}
                />
              </View>
              ...
              <View className="w-[48%]">
                <CustomTextInput
                  ...
                  validationIcon={
                    formData.cvc && !errors.cvc ? "checkmark-circle" : undefined
                  }
                  validationIconColor={COLORS.success}
                  showValidationIcon={!!formData.cvc && !errors.cvc}
                />
              </View>
            </View>
