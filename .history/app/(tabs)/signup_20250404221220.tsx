  ...
  return (
    <Wrapper>
      <View className="flex-1 w-full bg-white px-6 justify-start mt-8">
            ...
            <View className="flex-row h-1.5 bg-gray-200 rounded-full overflow-hidden">
              {Array.from({ length: 5 }).map((_, index) => (
                <View
                  ...
                  style={{
                    backgroundColor:
                      index < passwordStrength ? strengthColor : COLORS.gray,
        ...
        <Button
          state="primary"
          title={isLoading ? "" : "Buyer Account"}
          onPress={() => router.push("../welcome")}
          disabled={isLoading}
          accessibilityLabel="Buyer Account"
          accessibilityRole="button"
          className={`w-full py-4 rounded-[3px] bg-[#272d2d] flex items-center justify-center mt-2`}
        >
          {isLoading ? (
            <ActivityIndicator color="#fff" />
          ) : (
