    ...
    <ScrollView className="flex-1 bg-white px-4 pt-8">
      ...
      >
        {image ? (
          ...
          <Image
            source={require('../assets/upload-placeholder.png')} // <-- Replace with your local placeholder
      ...
      <View className="flex-row justify-between mb-4 bg-gray-100 rounded-lg p-2">
        <Pressable className="flex-1 items-center p-2 bg-white rounded-lg mr-2">
          {image && <Image source={{ uri: image }} className="w-16 h-16 rounded" />}
      ...
      <View className="flex-row justify-between mb-6">
        ...
        <View className="w-[48%] flex-row items-center justify-between border border-gray-300 rounded-md px-3">
          <Pressable onPress={() => setQuantity(Math.max(0, quantity - 1))} className="p-1">
            <Text className="text-xl">-</Text>
          </Pressable>
          <Text>{quantity}</Text>
          <Pressable onPress={() => setQuantity(quantity + 1)} className="p-1">
            <Text className="text-xl">+</Text>
          </Pressable>
        </View>
      </View>
      ...
      <View className="flex-row justify-between mb-12">
        <Pressable className="flex-1 bg-primary py-3 rounded-lg mr-2">
          <Text className="text-white text-center font-semibold text-sm">UPLOAD PRODUCT</Text>
        </Pressable>
        <Pressable className="flex-1 border border-primary py-3 rounded-lg ml-2">
          <Text className="text-primary text-center font-semibold text-sm">CANCEL</Text>
        </Pressable>
      </View>
    </ScrollView>
  );
