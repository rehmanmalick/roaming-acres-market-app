...
export default function EditProfile() {
  ...
  return (
    <ScrollView contentContainerStyle={{ flexGrow: 1 }} showsVerticalScrollIndicator={false}>
      <View style={styles.container} className="flex-1 bg-[#FDFDFD] items-center">
        {/* ImageBackground with dynamic source */}
        <ImageBackground source={backgroundImage} style={styles.imageBackground} resizeMode="cover">
          <TouchableOpacity onPress={() => router.back()} style={styles.back} className="absolute left-5 top-2 bg-white p-3">
            <Ionicons name="arrow-back" size={16} color="black" />
          </TouchableOpacity>
          <View className="items-center ">
            <View className=" ">
              {/* Profile image with dynamic source */}
              <Image source={profileImage} className="w-40 h-40 rounded-full" style={styles.img} />
            </View>
            <Text className="text-lg font-bold mt-2">Try Temp</Text>
            <Text className="text-sm text-gray-500">Joined Roaming Acres Market in</Text>
        ...
        <View className="flex flex-row justify-center items-center mt-10 ">
          <Button state="primary" title="Massage" onPress={() => router.push("../welcome")} />
        </View>
