...
const CheckoutComponent: React.FC<{ mainText: string; subText: string; icon: keyof typeof Ionicons.glyphMap; onPress?: () => void; TextIcon?: boolean }> = ({mainText, subText, icon, TextIcon, onPress}) => {
  return (
    <TouchableOpacity onPress={onPress} className="flex-row items-center justify-between p-4 bg-white mx-5 mt-4">
      ...
      { TextIcon ? <Ionicons name="chevron-forward" size={20} color="#9CA3AF" /> : <Text className="text-gray-500">View in maps</Text>} 
    </TouchableOpacity>
  );
};
